...
var Cesium = require("cesium"); // 自定义label 的显示
...
function () {
  function labelBillboard(viewer, position, HtmlResult, options) {
    ...
    this.id = options.type + options.id; // 判断传递过来的数据类型

    var elementObj = HtmlResult instanceof Function ? HtmlResult() : HtmlResult; // console.log(HtmlResult);
    // 经纬度不存在的时候直接return
    ...
    this.element = elementObj; // 获取相机高度

    this.maxRenderDis = Math.round(viewer.camera.positionCartographic.height) * 5;
    this.show = true;
    this.initBillboard();
  }

  labelBillboard.prototype.initBillboard = function () {
    var _this = this; // 创建一个div


    var userBox = document.createElement("div"); // userBox.id = this.id;

    userBox.style.position = "absolute";
    userBox.className = "resultMarker";
    userBox.innerHTML = JSON.stringify(this.element);
    this.element = userBox; // 自定义html

    this.viewer.cesiumWidget.container.appendChild(this.element); //实时更新 场景渲染完成执行的操作，会一直执行
    // this.viewer.scene.postRender.addEventListener(() => {
    //   this.updateBillboardLocation();
    // });
    // 第一次显示的时候显示

    this.updateBillboardLocation(); // 实时更新点位位置 相机位置移动时执行方法

    var previousCameraPosition = this.viewer.camera.position.clone();
    this.viewer.scene.postRender.addEventListener(function () {
      // 获取相机位置
      var currentCameraPosition = _this.viewer.camera.position; // 相机位置发生变化

      _this.updateBillboardLocation(); // 相机位置发生变化时触发label位置更新
  ...
  labelBillboard.prototype.updateBillboardLocation = function () {
    ...
    if (this.element) {
      ...
      if (this.show) {
        if (!(Cesium.Cartesian3.distance(camerPosition, this.position) > height) && this.viewer.camera.positionCartographic.height < this.maxRenderDis) {
