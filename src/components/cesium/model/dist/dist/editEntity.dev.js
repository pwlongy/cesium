...
function () {
  ...
  editEntity.prototype.linePlay = function (lineList, timeSpace, options, dataSourceName) {
    ...
    var viewer = this.dataSource[dataSourceName] || this.viewer; // 定义路径

    var property = new Cesium.SampledPositionProperty(); // 定义时间范围

    var startTime = new Date();
    var stopTime = startTime;
    var startTimeStamp = startTime.getTime(); // 先添加一条原有轨迹

    this.addPolyline(lineList, {}, 'ManageLine'); // 添加每一个时间段的位置
